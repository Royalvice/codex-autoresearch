...
- **`research-results.tsv`** -- 完全な監査証跡、イテレーションごとに 1 行
...
両方のファイルは git にコミットしません。セッション再開時、JSON 状態は TSV の行数とクロスバリデーションされ不整合を検出します。進捗サマリーは 5 イテレーションごとに出力されます。有界実行では最後にベースラインから最良値までのサマリーが出力されます。
